...
import { Timer, DollarSign, Globe, Play, Pause, RotateCcw, Target, Sparkles, Bot, Smartphone, Download } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const isLowTime = stats.secondsRemaining < 1800; // less than 30 mins
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 flex flex-wrap items-center justify-between gap-4 border-b border-stone-800/80 text-xs">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 font-bold tracking-tight text-amber-400">
            ...
            2-HOUR AGENCY SPRINT MODE
          </div>
          <span className="text-stone-600 hidden sm:inline">|</span>
          <span className="text-stone-400 hidden sm:inline">
            Workflow: Find 4.5★+ map businesses without websites → Copy info → Generate site → Charge $300-$500
          </span>
        </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border font-mono font-bold ${
            isLowTime ? 'bg-rose-950/70 border-rose-600 text-rose-300 animate-pulse' : 'bg-stone-800/90 border-stone-700 text-amber-300'
          }`}>
            <Timer className="w-3.5 h-3.5 text-amber-400" />
            <span>{formatTime(stats.secondsRemaining)}</span>
            <button
